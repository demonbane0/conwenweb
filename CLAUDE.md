...
- 圖檔名一律用 slug（ASCII），不用中文檔名
- `brand` 建議填寫：/products 頁有廠牌下拉篩選（支援 `?brand=` URL 參數，
  代理廠牌頁與首頁 logo 連到對應篩選）；廠牌顯示名須沿用既有值
  （完整清單見 `BRAND-REVIEW.md` 第三節），拼法不同會被視為不同廠牌
...
- **色彩**：主色 `#14467b`（深藍）、深色 `#0d2f54`、最深 `#081f3a`（footer/hero 底）、
  **CTA 橙 `#c2410c`**（hover `#9a3412`；白字對比 AA 合格）、裝飾橙線 `#e8590c`
  （僅用於粗體大字或非文字元素）、藍底淡色 `#e8eef6`、
  背景 `#ffffff` / 區塊底 `#f4f6f9`、文字 `#1f2933`、次要文字 `#5b6b7c`、邊框 `#dde3ea`
- **字級**：base 16px；h1 clamp(1.85rem,4vw,2.6rem)；行高 1.7；中文不用斜體；
